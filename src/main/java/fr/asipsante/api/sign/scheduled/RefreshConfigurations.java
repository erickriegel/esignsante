...
public class RefreshConfigurations {
     ...
     * Refresh crl.
     */
    @Scheduled(cron = "${config.crl.scheduling ?: 0 0 0 * * ?}")
    ...
    public void refreshCrlDefault() {
        if (cronConf == null || "".equals(cronConf)) {
            ...
            log.info("Votre expression cron est : {}, si elle n'est pas valide le téléchargement des CRLs se fera tout les jours à minuit", cronConf);
        }
    }
}
