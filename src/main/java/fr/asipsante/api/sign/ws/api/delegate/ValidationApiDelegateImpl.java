...
public class ValidationApiDelegateImpl extends ApiDelegate implements ValidationApiDelegate {
     ...
     */
    private ResponseEntity<RapportVerifWithProof> validateDigitalSignatureWithProof(Long idVerifSignConf,
            MultipartFile doc, String requestId, String proofTag, String applicantId, Long idProofConf,
            boolean isXades) {
        final Optional<String> acceptHeader = getAcceptHeader();
        ResponseEntity<RapportVerifWithProof> re = new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
        ...
        if (acceptHeader.isPresent() && acceptHeader.get().contains(WsVars.HEADER_TYPE.getVar())) {
            if (doc == null || idVerifSignConf == null || requestId == null || proofTag == null || applicantId == null
                ...
                final SignatureValidationParameters signValidationParameters =
                        params.getSignatureValidationConfigurations().get(idVerifSignConf.toString());
                final SignatureParameters signProofParams =
                        params.getProofSignatureConfigurations().get(idProofConf.toString());
                ...
                if (signValidationParameters == null || signProofParams == null) {
                    ...
                    try {
                        ...
                        if (status != HttpStatus.CONTINUE) {
                            ...
                            final RapportSignature rapportSignProofAsip =
                                    signatureService.signXMLDsig(proof, signProofParams);

                            final RapportVerifWithProof rapport = populateResultSignWithProof(
     ...
     */
    @Override
    public ResponseEntity<RapportVerifWithProof> verifSignatureXMLdsigWithProof(Long idVerifSignConf, MultipartFile doc,
            String requestId, String proofTag, String applicantId, Long idProofConf) {
     ...
     */
    @Override
    public ResponseEntity<RapportVerifWithProof> verifSignatureXadesWithProof(Long idVerifSignConf, MultipartFile doc,
            String requestId, String proofTag, String applicantId, Long idProofConf) {
        return validateDigitalSignatureWithProof(idVerifSignConf, doc, requestId, proofTag, applicantId, idProofConf,
                true);
    }
     ...
     */
    @Override
    public ResponseEntity<RapportVerifWithProof> verifCertificatWithProof(Long idVerifCertConf, MultipartFile doc,
            String requestId, String proofTag, String applicantId, Long idProofConf) {
        ...
        if (acceptHeader.isPresent() && acceptHeader.get().contains(WsVars.HEADER_TYPE.getVar())) {
            if (doc == null || idVerifCertConf == null || requestId == null || proofTag == null || applicantId == null
                ...
                final CertificateValidationParameters certValidationParameters =
                        params.getCertificateValidationConfigurations().get(idVerifCertConf.toString());
                final SignatureParameters signProofParams =
                        params.getProofSignatureConfigurations().get(idProofConf.toString());
                final ProofParameters proofParameters =
                        new ProofParameters("VerifCert", requestId, proofTag, applicantId);
                ...
                if (certValidationParameters == null) {
                    ...
                    try {
                        if (isBinaryFile(doc)) {
                            ...
                            final String docString = new String(doc.getBytes(), StandardCharsets.UTF_8);
                            rapportVerifCertAsip = certificateValidationService.validateCertificat(docString,
                                    certValidationParameters, serviceCaCrl.getCacrlWrapper());
                        }
                        ...
                        if (status != HttpStatus.CONTINUE) {
                            ...
                            // Signature de la preuve
                            final RapportSignature rapportSignProofAsip =
                                    signatureService.signXADESBaselineB(proof, signProofParams);

                            final RapportVerifWithProof rapport = populateResultVerifCertWithProof(
     ...
     */
    @Override
    public ResponseEntity<RapportVerif> verifCertificat(Long idVerifCertConf, MultipartFile doc) {
        final Optional<String> acceptHeader = getAcceptHeader();
        ResponseEntity<RapportVerif> re = new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
            ...
            if (doc == null || idVerifCertConf == null) {
                ...
                if (certValidationParameters == null) {
                    ...
                    try {
                        ...
                        final RapportVerif rapport =
                                populateResultVerifCert(rapportVerifCertAsip.getListeErreurCertificat(),
                                        rapportVerifCertAsip.getMetaData(), rapportVerifCertAsip.isValide());
     ...
     * @return the response entity
     */
    private ResponseEntity<RapportVerif> validateDigitalSignature(Long idVerifSignConf, MultipartFile doc,
            boolean isXades) {
        final Optional<String> acceptHeader = getAcceptHeader();
        ResponseEntity<RapportVerif> re = new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
            ...
            if (doc == null || idVerifSignConf == null) {
                ...
                if (signValidationParameters == null) {
                    ...
                    try {
                        ...
                        final RapportVerif rapport = populateResultSign(rapportVerifSignAsip.getListeErreurSignature(),
     ...
     */
    @Override
    public ResponseEntity<RapportVerif> verifSignatureXMLdsig(Long idVerifSignConf, MultipartFile doc) {
     ...
     */
    @Override
    public ResponseEntity<RapportVerif> verifSignatureXades(Long idVerifSignConf, MultipartFile doc) {
     ...
     */
    private RapportVerif populateResultSign(List<ErreurSignature> erreursSignature, List<MetaDatum> metadata,
            boolean isValide) {
        final RapportVerif rapport = new RapportVerif();
     ...
     */
    private RapportVerifWithProof populateResultSignWithProof(List<ErreurSignature> erreursSignature,
            List<MetaDatum> metadata, boolean isValide, String preuve) {
        final RapportVerifWithProof rapport = new RapportVerifWithProof();
     ...
     * @return the rapport verif
     */
    private RapportVerif populateResultVerifCert(List<ErreurCertificat> erreursVerifCert, List<MetaDatum> metadata,
            boolean isValide) {
        final RapportVerif rapport = new RapportVerif();
     ...
     * @return the rapport verif with proof
     */
    private RapportVerifWithProof populateResultVerifCertWithProof(List<ErreurCertificat> erreursVerifCert,
