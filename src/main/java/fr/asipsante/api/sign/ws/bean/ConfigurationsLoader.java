...
public class ConfigurationsLoader {
    ...
    public void load(Parameters paramsList) {
        ...
        for (final String signId : signIds) {
            final String signPathValue = prop.getProperty(CONF_PREFIX + signId + WsVars.PROPS_PATH.getVar());
            final String signProofId = prop.getProperty(CONF_PREFIX + signId + WsVars.PROPS_PROOF.getVar());
            ...
            try {
                ...
                if ("enable".equalsIgnoreCase(secretEnabled)) {
                    final String[] signSecrets = prop
                            .getProperty(CONF_PREFIX + signId + WsVars.PROPS_SECRET.getVar()).trim().split(" ");
