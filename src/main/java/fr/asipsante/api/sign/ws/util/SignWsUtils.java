...
public class SignWsUtils {
     ...
     */
    private static List<X509Certificate> getSignatureCertificates(
            final File pkcs12File, final String password)
     ...
     *             the asip sign exception
     */
    public static HttpStatus checkCertificate(

            SignatureParameters signParams, CACRLWrapper caCrlWrapper)
            throws AsipSignException {
        // On contrôle le certificat qui va signer
        List<X509Certificate> certificats = null;
        try {
            if (signParams != null) {
                certificats = getSignatureCertificates(signParams.getKsFile(),
                        signParams.getPassword());
            } else {
                return HttpStatus.SERVICE_UNAVAILABLE;
            }
        ...
        ICertificateValidationService certificatevValidationService = new CertificateValidationServiceImpl();
        List<ICertificatVisitor> certRules = new ArrayList<>();
        ...
        CertificateValidationParameters certParams = new CertificateValidationParameters();
        certParams.setRules(certRules);
        // On boucle sur tous les certificats du Keystore
        // et on lance les règles de validation
        for (X509Certificate cert : certificats) {
            try {
                RapportValidationCertificat rapportValidationCert = certificatevValidationService
                        .validateCertificat(cert.getEncoded(), certParams,
                                caCrlWrapper);
                if (!rapportValidationCert.isValide()) {
                    String error = rapportValidationCert
                            .getListeErreurCertificat().toString();
                    LOG.error(error);
                    return HttpStatus.SERVICE_UNAVAILABLE;
                }
