...
public class SignaturesApiDelegateImpl extends ApiDelegate implements SignaturesApiDelegate {
     ...
     */
    private ResponseEntity<AsipSignatureReportWithProof> digitalSignatureWithProof(
        ...
        if (!signConf.isPresent() || !verifConf.isPresent()) {
            ...
            if (acceptHeader.isPresent() && acceptHeader.get().contains(WsVars.HEADER_TYPE.getVar())) {
                ...
                if (signParamsMissing(idSignConf, doc, idVerifSignConf) || proofParamsMissing(proofParameters)) {
                    ...
                    final SignatureValidationParameters signVerifParams = ConfigurationLoader.loadSignVerifConf(
                            verifConf.get());
                    final SignatureParameters signProofParams = ConfigurationLoader.loadProofSignConf(
                            signProofConf.get());
     ...
     * @return the response entity
     */
    private ResponseEntity<AsipSignatureReportWithProof> signWithProof(
     ...
     */
    @Override
    public ResponseEntity<AsipSignatureReportWithProof> signatureXMLdsigWithProof(
     ...
     */
    @Override
    public ResponseEntity<AsipSignatureReportWithProof> signatureXadesWithProof(
            final Long idSignConf, final MultipartFile doc, final Long idVerifSignConf, final String requestId,
            final String proofTag, final String applicantId, final String secret) {
        Version wsVersion = DEFAULT_VERSION;
        try {
     ...
     * @return the response entity
     */
    private ResponseEntity<AsipSignatureReport> digitalSignature(final String secret, final Long idSignConf,
     ...
     * @return the response entity
     */
    private ResponseEntity<AsipSignatureReport> sign(final SignatureParameters signParams, final MultipartFile doc,
     ...
     */
    @Override
    public ResponseEntity<AsipSignatureReport> signatureXMLdsig(final Long idSignConf, final MultipartFile doc,
                                                                final String secret) {
     ...
     */
    @Override
    public ResponseEntity<AsipSignatureReport> signatureXades(final Long idSignConf, final MultipartFile doc,
                                                              final String secret) {
     ...
     */
    private AsipSignatureReport populateResultSign(final List<ErreurSignature> erreursSignature,
     ...
     */
    private AsipSignatureReportWithProof populateResultSignWithProof(
