...
public class SignaturesApiDelegateImpl extends ApiDelegate implements SignaturesApiDelegate {
     ...
     */
    private ResponseEntity<RapportSignatureWithProof> digitalSignatureWithProof(Long idSignConf, MultipartFile doc,
            Long idVerifSignConf, String requestId, String proofTag, String applicantId, Long idProofConf,
            boolean isXades) {

        final Optional<String> acceptHeader = getAcceptHeader();
        ResponseEntity<RapportSignatureWithProof> re = new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);

        if (acceptHeader.isPresent() && acceptHeader.get().contains(WsVars.HEADER_TYPE.getVar())) {
            if (idSignConf == null || doc == null || idVerifSignConf == null || requestId == null || proofTag == null
                    ...
                    try (InputStreamReader reader = new InputStreamReader(doc.getInputStream())) {
                        ...
                        if (status != HttpStatus.CONTINUE) {
                            ...
                            if (status != HttpStatus.CONTINUE) {
                                ...
                                final RapportSignature rapportSignaturePreuve =
                                        signatureService.signXADESBaselineB(proof, signProofParams);

                                final RapportSignatureWithProof rapport =
                                        populateResultSignWithProof(rapportVerifSignature.getListeErreurSignature(),
     ...
     */
    @Override
    public ResponseEntity<RapportSignatureWithProof> signatureXMLdsigWithProof(Long idSignConf, MultipartFile doc,
            Long idVerifSignConf, String requestId, String proofTag, String applicantId, Long idProofConf) {
     ...
     */
    @Override
    public ResponseEntity<RapportSignatureWithProof> signatureXadesWithProof(Long idSignConf, MultipartFile doc,
            Long idVerifSignConf, String requestId, String proofTag, String applicantId, Long idProofConf) {
        return digitalSignatureWithProof(idSignConf, doc, idVerifSignConf, requestId, proofTag, applicantId,
                idProofConf, true);
    }
     ...
     * @return the response entity
     */
    private ResponseEntity<fr.asipsante.api.sign.ws.model.RapportSignature> digitalSignature(Long idSignConf,
            MultipartFile doc, boolean isXades) {
        final Optional<String> acceptHeader = getAcceptHeader();
        ResponseEntity<fr.asipsante.api.sign.ws.model.RapportSignature> re =
            ...
            if (idSignConf == null || doc == null) {
                ...
                try {
                    ...
                    final HttpStatus status =
                            SignWsUtils.checkCertificate(params.getSignatureConfigurations().get(idSignConf.toString()),
                                    serviceCaCrl.getCacrlWrapper());
                    if (status != HttpStatus.CONTINUE) {
                        re = new ResponseEntity<>(status);
                    } else {
                        ...
                        final fr.asipsante.api.sign.ws.model.RapportSignature rapport = populateResultSign(
     ...
     */
    @Override
    public ResponseEntity<fr.asipsante.api.sign.ws.model.RapportSignature> signatureXMLdsig(Long idSignConf,
            MultipartFile doc) {
     ...
     */
    @Override
    public ResponseEntity<fr.asipsante.api.sign.ws.model.RapportSignature> signatureXades(Long idSignConf,
            MultipartFile doc) {
     ...
     */
    private fr.asipsante.api.sign.ws.model.RapportSignature populateResultSign(List<ErreurSignature> erreursSignature,
            String signedDocument) {
        final fr.asipsante.api.sign.ws.model.RapportSignature rapport =
                new fr.asipsante.api.sign.ws.model.RapportSignature();
     ...
     */
    private RapportSignatureWithProof populateResultSignWithProof(List<ErreurSignature> erreursSignature,
            List<MetaDatum> metadata, boolean isValide, String signedDocument, String preuve) {
        final RapportSignatureWithProof rapport = new RapportSignatureWithProof();
