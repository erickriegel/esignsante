...
public class ConfigurationLoaderTest {
    ...
    public static void init() throws Exception {
        wsConfPathGood = Thread.currentThread().getContextClassLoader()
                .getResource("asip-sign-ws.properties").toURI().getPath();
        wsConfPathBad = Thread.currentThread().getContextClassLoader()
                .getResource("asip-sign-ws-test-bad-configs.properties").toURI()
                .getPath();
    }
    ...
    public void configsOkTest() {
        ...
        loader.load(params);
        assertFalse("La liste des configs de signature est vide",
                params.getSignatureConfigurations().isEmpty());
        assertEquals(
                "La liste ne contient pas le bon nombre de configuration de signature",
                5, params.getSignatureConfigurations().size());
    }
