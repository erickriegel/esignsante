...
public class SignatureParametersLoadTest {
    ...
    private static final Logger LOG = LoggerFactory
            .getLogger(SignatureParametersLoadTest.class);
    ...
    public void getSignPackagingTest() throws Exception {
        ...
        assertTrue(params.getSignPackaging() != null);
        assertTrue(params.getSignId() != null);
    ...
    public void testRulesDefinitions() throws Exception {
        ...
        IVisitor rule = new TrustedCertificat();
        ...
        List<IVisitor> rules = new ArrayList<IVisitor>();
        rules.add(rule);
        params.setRules(rules);
        params.setMetaData(metadata);
        Map<String, SignatureValidationParameters> map = new HashMap<String, SignatureValidationParameters>();
        map.put("1", params);
        List<ConfVerifSign> liste = mapVerifSignConfig(map);
        for (ConfVerifSign confVerifSign : liste) {
            assertTrue(confVerifSign.getRules().get(0).getDescription()
                    .equals("L'autorité de Certification est reconnue."));
        }
